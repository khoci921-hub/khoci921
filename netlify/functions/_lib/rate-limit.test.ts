...
import { check, fail, _clearForTests } from './rate-limit';
...
describe('rate-limit — full store (FIX #7)', () => {
  it('does not crash when store is full with active buckets', () => {
    // Fill store with MAX_BUCKETS active buckets
    for (let i = 0; i < 20000; i++) {
      check('fill-' + i, { limit: 5, windowMs: 60000 });
    }
    // New request should not crash — LRU eviction makes room
    const result = check('overflow-key', { limit: 5, windowMs: 60000 });
    expect(result.ok).toBe(true);
  });

  it('evicts oldest bucket when store is full', () => {
    // Fill store
    for (let i = 0; i < 20000; i++) {
      check('evict-' + i, { limit: 5, windowMs: 60000 });
    }
    // The oldest key (evict-0) should be evicted to make room
    // New request creates room via LRU eviction
    check('evict-0', { limit: 5, windowMs: 60000 });
    // Should not crash
    const result = check('after-evict', { limit: 5, windowMs: 60000 });
    expect(result.ok).toBe(true);
  });

  it('fail() does not crash on full store', () => {
    for (let i = 0; i < 20000; i++) {
      check('fail-full-' + i, { limit: 5, windowMs: 60000 });
    }
    // fail() should not crash
    fail('fail-overflow', { windowMs: 60000, lockoutAfter: 3, lockoutMs: 30000 });
    const result = check('fail-overflow', { limit: 5, windowMs: 60000 });
    expect(result.ok).toBe(true);
  });
});
